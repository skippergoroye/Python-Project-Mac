...
def request_form(request):
	return render(request, 'publisher/user_request2.html')


@login_required
def feedback_form(request):
	return render(request, 'publisher/user_feedback2.html')

@login_required
def about(request):
	return render(request, 'publisher/about2.html')	
...
def usearch(request):
    ...
    if( len(data) == 0):
        return redirect('publisher')
                ...
                # Searching for It
                qs5 =models.Book.objects.filter(course_code__iexact=a).distinct()
                qs6 =models.Book.objects.filter(course_code__exact=a).distinct()

                qs7 =models.Book.objects.all().filter(course_code__contains=a)
                qs8 =models.Book.objects.select_related().filter(course_code__contains=a).distinct()
                qs9 =models.Book.objects.filter(course_code__startswith=a).distinct()
                qs10 =models.Book.objects.filter(course_code__endswith=a).distinct()
                qs11 =models.Book.objects.filter(course_code__istartswith=a).distinct()
                qs12 =models.Book.objects.all().filter(course_code__icontains=a)
                qs13 =models.Book.objects.filter(course_code__iendswith=a).distinct()
...
class UBookListView(LoginRequiredMixin,ListView):
	model = Book
	template_name = 'publisher/book_list2.html'
...
class UListChat(LoginRequiredMixin, ListView):
	model = Chat
	template_name = 'publisher/chat_list2.html'
...
def labook(request):
	if request.method == 'POST':
		...
		a = Book(title=title,  course_code=course_code, 																																																								author=author, year=year, publisher=publisher, 
			desc=desc, cover=cover, pdf=pdf, uploaded_by=username, user_id=user_id)
		a.save()
		messages.success(request, 'Book was uploaded successfully')
